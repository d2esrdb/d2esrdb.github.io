...
class Property:
    def __init__(self, utils, code, param, min, max, chance=""):
        ...
        if code == "dmg%":
            ...
            stat.stat_string = (
                ...
                + "% "
                + self.get_descstr("strModEnhancedDamage")
            )
    ...
    def get_property_value_string(self, stat):
        ...
        match stat.func:
            case "1" | "3" | "5" | "6" | "7" | "8" | "9" | "10" | "22" | "24":
                ...
                return (
                    "("
                    + self.strmin(self.min, self.max)
                    + " to "
                    + self.strmax(self.min, self.max)
                    + ")"
                )
            case "2" | "4" | "16":
                return self.max
            ...
            case "17":
                ...
                if self.min == "":
                    return (
                        "("
                        + self.strmin(0, self.max)
                        + " to "
                        + self.strmax(0, self.max)
                        + ")"
                    )
                if self.max == "":
                    return (
                        "("
                        + self.strmin(self.min, 0)
                        + " to "
                        + self.strmax(self.min, 0)
                        + ")"
                    )
                if self.min == self.max:
                    return self.min
            ...
            case _:
                self.utils.log(
                    "Could not get property value string for stat "
                    + stat.stat
                    + " func "
                    + stat.func
                )
    ...
    def get_descstr(self, descstr):
        ...
        if "NOT FOUND" == self.utils.table_strings.get(descstr, "NOT FOUND"):
            self.utils.log(
                "No descstr found for: " + descstr + " property:" + self.code
            )
            return descstr
    ...
    def get_skill_name_from_skill_id(self, skill_id):
        for s in self.utils.tables.skills_table:
            if (
                s["skill"].lower() == skill_id.lower()
                or s["Id"].lower() == skill_id.lower()
            ):
                        ...
                        for cs in self.utils.tables.char_stats_table:
                            if pc["Player Class"] == cs["class"]:
                                ...
                                return self.utils.table_strings.get(
                                    cs["StrClassOnly"], "(Unassigned Only)"
                                )
        ...
        match stat.descfunc:
            ...
            case "16":
                return stat.descstr.replace(
                    "%d", self.get_property_value_string(stat), 1
                ).replace("%s", self.get_skill_name_from_skill_id(self.param), 1)
            case "19":
                return (
            ...
            case "23":
                # %0%% Reanimate as: %1
                return stat.descstr.replace(
                    "%0%", self.get_property_value_string(stat)
                ).replace("%1", self.utils.get_monster_from_id(self.param))
            case "24":
                return (
                    "Level "
                    + self.max
            ...
            case _:
                self.utils.log(
                    "descfunc"
                    + stat.descfunc
                    + " descval 0 not implemented (stat "
                    + stat.stat
                    + ")"
                )
        ...
        match stat.descfunc:
            ...
            case "23":
                return (
                    ...
                    + stat.descstr
                    + " "
                    + self.utils.get_monster_from_id(self.param)
                )
            case _:
                self.utils.log(
                    "descfunc"
                    + stat.descfunc
                    + " descval 1 not implemented (stat "
                    + stat.stat
                    + ")"
                )
        ...
        match stat.descfunc:
            ...
            case _:
                self.utils.log(
                    "descfunc"
                    + stat.descfunc
                    + " descval 2 not implemented (stat "
                    + stat.stat
                    + ")"
                )
