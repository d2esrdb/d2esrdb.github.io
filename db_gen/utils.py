...
def get_skill_name(skill):
    ...
    for skill_desc_row in skill_desc_table:
...
def get_gamble_item_from_code(code):
    ...
    for row in weapons_table:
        if row[3] == code and row[9] == str(1):
            for row_again in weapons_table:
...
def get_stat(stat_name, param, min, max, prop_name):
    for item_stat_cost_row in item_stat_cost_table:
...
def fill_property_stats(property):
    ...
    foundone = False
    for property_row in properties_table:
