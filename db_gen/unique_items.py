...
def get_unique_items(tables, mod_strings, include_implicits):
    unique_items = []
    for row in tables.unique_items_table:
        ...
        if row["rarity"].isdigit() and int(row["rarity"]) > 0:
            ...
            unique_items.append(Item(row["index"], row["lvl"], row["lvl req"], properties, row["code"], mod_strings, tables, include_implicits))
