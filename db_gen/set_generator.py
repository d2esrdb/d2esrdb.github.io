...
class Set:
    def __init__(self, utils, name, items, partial_bonus_properties, full_bonus_properties):
        self.utils = utils
        self.name = name
        self.items = items
...
class Set_Item:
    def __init__(self, utils, name, base_code, base_name, gamble_string, required_level, level, item_properties, bonus_properties, base_url):
        self.utils = utils
        self.name = name
        ...
        for _set in self.utils.tables.sets_table:
            ...
            for i in range(4):
                if _set["PCode" + str(i+2) + "a"] != "":
                    partial_properties[i].append(properties.Property(self.utils,
            ...
            for i in range(8):
                if _set["FCode" + str(i+1)] != "":
                    full_properties.append(properties.Property(self.utils,
                                                               _set["FCode" + str(i+1)],
                                                               _set["FParam" + str(i+1)],
                                                               _set["FMin" + str(i+1)],
                                                               _set["FMax" + str(i+1)]))
                ...
                if item["set"] == _set["index"]:
                    ...
                    for i in range(5):
                        if item["aprop" + str(i+1) + "a"] != "":
                            item_partial_properties[i].append(properties.Property(self.utils,
                    ...
                    for i in range(9):
                        if item["prop" + str(i+1)] != "":
                            item_properties.append(properties.Property(self.utils,
                                                                       item["prop" + str(i+1)],
                                                                       item["par" + str(i+1)],
                                                                       item["min" + str(i+1)],
                                                                       item["max" + str(i+1)]))
                    ...
                    items.append(Set_Item(self.utils, item_name, item_base_code, item_base_name, gamble_string, item_required_level, item_level, item_properties, item_partial_properties, self.utils.get_base_url(item_base_code)))
            sets.append(Set(self.utils, name, items, partial_properties, full_properties))
        return sets
